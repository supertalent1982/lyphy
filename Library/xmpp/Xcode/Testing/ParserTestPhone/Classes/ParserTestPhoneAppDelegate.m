...
@implementation ParserTestPhoneAppDelegate
...
- (NSString *)stringToParse:(BOOL)full
{
	...
	[mStr appendString:@"  <stream:features>"];
...
- (void)nonParserTest
{
	...
	//	[str release];
		...
		DDXMLDocument *doc = [[DDXMLDocument alloc] initWithData:mData options:0 error:nil];
		if(doc)
		{
			if(PRINT_ELEMENTS)
			{
